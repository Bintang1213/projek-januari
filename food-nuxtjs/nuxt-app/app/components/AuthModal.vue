<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div class="bg-white w-full max-w-sm rounded-2xl p-8 shadow-xl relative z-10 border border-gray-100">
...
const handleAuth = async () => {
  ...
  try {
    const data = await apiFetch(endpoint, { method: 'POST', body: form.value })

    if (isLogin.value) {
      localStorage.setItem('token', data.token)
      localStorage.setItem('user', JSON.stringify(data))
      emit('login-success', data)
      toast.success(`Halo ${data.name}, berhasil masuk!`)
      ...
      toast.info('Akun berhasil dibuat! Silakan masuk.')
      isLogin.value = true
    }
  } catch (err) {
    toast.error(err.response?._data?.message || 'Gagal masuk.', { theme: 'colored' })
  }
}
</script>